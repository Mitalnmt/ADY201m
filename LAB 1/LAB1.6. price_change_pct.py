...
# --- Load dữ liệu từ DB hoặc file ---
conn = sqlite3.connect('data/aapl_analysis.db')
...
# Chuyển kiểu datetime nếu cần
df_sentiment_analysis['earnings_date'] = pd.to_datetime(df_sentiment_analysis['earnings_date'])

# 1. Chuẩn bị dữ liệu
X = df_sentiment_analysis[['compound']]  # đặc trưng
y = df_sentiment_analysis['price_change_pct']  # biến mục tiêu
...
df_sentiment_analysis.reset_index(inplace=True)  # Nếu earnings_date là index
output_path = 'data/aapl_sentiment_analysis.csv'
df_sentiment_analysis.to_csv(output_path, index=False)
